...
impl Transactions {
    ...
    pub async fn simulate(self, txn: SignedTransaction) -> Result<impl Reply, Error> {
        ...
        let user_txn = aptos_types::transaction::Transaction::UserTransaction(txn.clone());
        let res = AptosVM::execute_block_and_keep_vm_status(vec![user_txn.clone()], state_view);
        match res {
            Ok(outputs) => {
                let version = self.ledger_info.version();
                let (_vm_status, tx_output) = &outputs[0];
                let exe_status = match tx_output.status() {
                    TransactionStatus::Keep(exec_status) => exec_status.clone(),
                ...
                let simulated_txn = TransactionOnChainData {
                    ...
                    info,
                    events: tx_output.events().to_vec(),
                    accumulator_root_hash: HashValue::default(),
                    changes: tx_output.write_set().clone(),
                };

                self.render_transactions(vec![simulated_txn])
            }
            Err(vm_status) => Err(Error::bad_request(format!(
                "transaction is rejected: {}",
                vm_status
            ))),
        }
